...
u8 SP0718_Initialize_from_T_Flash()
{
	...
	fp = filp_open("/mnt/sdcard/SP0718_sd", O_RDONLY , 0); 
	if (IS_ERR(fp)) { 
	...
	while (curr_ptr < (data_buff + file_size))
	{
		...
		if (strcmp((const char *)func_ind, "REG") == 0)		/* REG */
		{
			...
			SP0718_Init_Reg[i].init_reg = strtol((const char *)curr_ptr, 16);
			curr_ptr += 5;	/* Skip "00, 0x" */

			SP0718_Init_Reg[i].init_val = strtol((const char *)curr_ptr, 16);
			curr_ptr += 4;	/* Skip "00);" */
		...
		else									/* DLY */
		{
			...
			SP0718_Init_Reg[i].init_val = strtol((const char *)curr_ptr,  10);	/* Get the delay ticks, the delay should less then 50 */
		}
		i++;
	...
	for (j=0; j<i; j++)
	{
		...
		else if (SP0718_Init_Reg[j].op_code == SP0718_OP_CODE_REG)
		{
			...
			SP0718_write_cmos_sensor(SP0718_Init_Reg[j].init_reg, SP0718_Init_Reg[j].init_val);
		}
		else
		{
			printk("REG ERROR!\n");
...
void SP0718_Sensor_Init(void)
{
	...
	SP0718_write_cmos_sensor(0xd7,0x24);//10//2x根据增益判断轮廓阈值
	SP0718_write_cmos_sensor(0xd8,0x38);//24//1A//4x
	...
	SP0718_write_cmos_sensor(0xea,0x2c);
	...
	//dns       
	SP0718_write_cmos_sensor(0xfd,0x01);
	SP0718_write_cmos_sensor(0x64,0x76);//沿方向边缘平滑力度  //0-最强，8-最弱
	SP0718_write_cmos_sensor(0x65,0x32);
	...
	#if defined(PROJECT_M713_DRV_SUPPORT)
	SP0718_write_cmos_sensor(0xfd,0x00);
...
UINT32 SP0718_Open(void)
{
	...
	Sleep(10);
#ifdef DEBUG_SENSOR_SP0718  //gepeiwei   120903
...
{
	kal_uint16 SP0718_sensor_ev = 0,SP0718_sensor_bl= 0;
	
	SP0718_write_cmos_sensor(0xfd, 0x00);

	SP0718_sensor_ev = (SP0718_read_cmos_sensor(0x03)<<8)|SP0718_read_cmos_sensor(0x04);//读取曝光时间

	printk("wax->SP0718ev = %x,%x,%x\n",SP0718_read_cmos_sensor(0x03),SP0718_read_cmos_sensor(0x04),SP0718_sensor_ev);
	if (SP0718_sensor_ev > 0x274) //在较暗的场景下测试读取的曝光时间的实际值（越暗值越大）
	{	
		SP0718_write_cmos_sensor(0xfd, 0x01);
		SP0718_sensor_bl = SP0718_read_cmos_sensor(0xf1);
		//由于0718在稍暗一点的场景下曝光时间的实际值就会达到最大，且在室内环境下该值变化不是很敏感。于是再度一个p1页的0xf1即亮度统计值。
		SENSORDB("wax->SP0718_sensor_bl = %x\n",SP0718_sensor_bl);
		if(SP0718_sensor_bl <0x42) 
