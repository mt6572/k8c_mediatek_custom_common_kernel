...
//};
static struct i2c_board_info __initdata bma_i2c_info={ I2C_BOARD_INFO(BMA_DEV_NAME, 0x68)};
//static struct i2c_board_info __initdata i2c_BMA222={ I2C_BOARD_INFO("BMA222", 0x68)};
...
#if 0//def CUST_GYR_SENSOR
static int bma_read_gyrraw_data(struct i2c_client *client, s16 data[BMA_AXES_NUM])
{
	...
	if (priv->sensor_type == BMI160_TYPE) {/* BMA280 */
		u8 buf_tmp[BMA_DATA_LEN] = {0};
		//u8 buf_tmp[2] = {0};
		u8 buf_tmp2[2] = {0};
		//err = bma_i2c_read_block(client, BMI160_USER_DATA_14_ADDR,
				//buf_tmp, 6);
		
		//err = bma_i2c_read_block(client, BMI160_USER_DATA_8_ADDR,
			//	buf_tmp, 6);
		err = bma_i2c_read_block(client, BMI160_USER_STEP_COUNT_0_ADDR,
				buf_tmp2, 2);
		printk("bmi160 step === %d,%d\n\n",buf_tmp2[0],buf_tmp2[1]);
		if (err) {
			GSE_ERR("[%s]read raw data failed, err = %d\n",
			priv->sensor_name, err);
			return err;
		}
		data[BMA_AXIS_X] =0;//(s16)(((u16)buf_tmp2[1]) << 8 | buf_tmp2[0]);
		data[BMA_AXIS_Y] = 0;//123;//(s16)(((u16)buf_tmp[3]) << 8 | buf_tmp[2]) ;
		data[BMA_AXIS_Z] = 0;//456;//(s16)(((u16)buf_tmp[5]) << 8 | buf_tmp[4]);
               printk("ggggggg bmi160 [%s][16bit raw]"
...
#endif
//heyong add step
#define MODEM_SWITCH_PROC_NAME "bm160_step_data"
 #define PROC_NAME "bm160step"
 int bm160_step_data = 0;
 static int step_read(char *buf, char **start, off_t off, int count, int *eof, void *data)
{
    int len = 0;
    char *p = buf;
...
static int step_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	char tmp_buf[11] = {0};
	uint32_t copysize;
	int write_data = 0;
 ...
 {    
    ...
    step_dir = proc_mkdir(PROC_NAME, NULL);
    if (!step_dir)
    {
    ...
    {
       // entry = create_proc_entry(MODEM_SWITCH_PROC_NAME, S_IRUGO | S_IWUSR, step_dir);
       entry = create_proc_entry(MODEM_SWITCH_PROC_NAME, 0777, step_dir);
        if (entry)
        {
            entry->read_proc = step_read;
            entry->write_proc = step_write;
        }
    }

    //printk("bmi160 step === okkkkkkk");
   // return 0;
}
...
//heyong add reset ic
#define IC_MODEM_SWITCH_PROC_NAME "bm160_reset"
 #define IC_PROC_NAME "bm160reset"
 int bm160_reset = 0;
...
static int bma_read_raw_data(struct i2c_client *client, s16 data[BMA_AXES_NUM])
{
	...
	if (priv->sensor_type == BMA220_TYPE) {/* BMA220 */
	...
	}   else if (priv->sensor_type == BMI160_TYPE) {/* BMA280 */
		u8 buf_tmp[BMA_DATA_LEN] = {0};
		//u8 buf_tmp[2] = {0};
		u8 buf_tmp2[2] = {0};
		//u8 buf_tmp3[6] = {0};
		...
		err = bma_i2c_read_block(client, BMI160_USER_STEP_COUNT_0_ADDR,
				buf_tmp2, 2);
	#if 0//def TAP_FUCTION
		err = bma_i2c_read_block(client, 0x1c,
				&tap_staus, 1);
		err = bma_i2c_read_block(client, 0x1e,
				&tap_staus_1, 1);
		err = bma_i2c_read_block(client, 0x1f,
				&tap_staus_2, 1);
		tap_staus_data = tap_staus&0x08;
		printk("bmi160 step === %d,%d,%d,%d,%d\n\n",buf_tmp2[0],buf_tmp2[1],tap_staus,tap_staus_1,tap_staus_2);
		...
		//}
		#endif
		printk("bm160_reset step === %d\n\n",bm160_reset);
	      if(bm160_reset)
	      	{
		   tmp = 0xb2;
		   bm160_reset = 0;
	          err = bma_i2c_write_block(client,BMI160_CMD_COMMANDS_ADDR,&tmp,0x1);		   
	      	}
		  
		bm160_step_data =(s16)(((u16)buf_tmp2[1]) << 8 | buf_tmp2[0]);
		printk("bmi160 step === %d,%d,%d\n\n",buf_tmp2[0],buf_tmp2[1],bm160_step_data);
		...
		data[BMA_AXIS_X] =(s16)(((u16)buf_tmp[1]) << 8 | buf_tmp[0]);
		data[BMA_AXIS_Y] = (s16)(((u16)buf_tmp[3]) << 8 | buf_tmp[2]) ;
		data[BMA_AXIS_Z] = (s16)(((u16)buf_tmp[5]) << 8 | buf_tmp[4]);
               printk("bmi160 [%s][16bit raw]"
			"[%08X %08X %08X] => [%5d %5d %5d]\n",
			priv->sensor_name,
			data[BMA_AXIS_X],
			data[BMA_AXIS_Y],
...
static int bma_get_chip_type(struct i2c_client *client)
{
	...
	case BM160_CHIP_ID:
		obj->sensor_type = BMI160_TYPE;
		strcpy(obj->sensor_name, "bma160");
		break;
	default:
...
static int bma_set_range(struct i2c_client *client, enum BMA_RANGE_ENUM range)
{
	...
	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
	...
	}   else if (obj->sensor_type == BMI160_TYPE) {/* BMA280 */
		if (range == BMA_RANGE_2G)
			actual_range = BMI160_ACCEL_RANGE_2G;
		else if (range == BMA_RANGE_4G)
			actual_range = BMI160_ACCEL_RANGE_4G;
		else if (range == BMA_RANGE_8G)
			actual_range = BMI160_ACCEL_RANGE_8G;
		else {
			err = -EINVAL;
			GSE_ERR("invalid range = %d\n", range);
			mutex_unlock(&obj->lock);
		...
		err = bma_i2c_read_block(client,
			BMI160_USER_ACC_RANGE__REG, &data, 1);
		data = BMA_SET_BITSLICE(data,
			BMI160_USER_ACC_RANGE, actual_range);
		err += bma_i2c_write_block(client,
			BMI160_USER_ACC_RANGE__REG, &data, 1);
		mdelay(1);
		...
		if (err < 0)
			...
			switch (range) {
			case BMA_RANGE_2G:
				obj->sensitivity = 16384;
			break;
			case BMA_RANGE_4G:
				obj->sensitivity = 8192;
			break;
			case BMA_RANGE_8G:
...
#if 0
static int bmi160_set_step_counter_enable(struct i2c_client *client,u8 v_step_counter_u8)
{
u8 com_rslt = 0;
u8 v_data_u8 = 0;

	if (v_step_counter_u8 < 3) {
	
	com_rslt = bma_i2c_read_block(client,BMI160_USER_STEP_CONFIG_1_STEP_COUNT_ENABLE__REG,&v_data_u8, 1);
		//if (com_rslt == SUCCESS) {
			v_data_u8 =
			BMA_SET_BITSLICE(v_data_u8,
...
static int  bmi160_get_step_detector_enable(struct i2c_client *client,u8 *v_step_intr_u8)
{
	u8 com_rslt = 0;
	u8 v_data_u8 = 0;

			com_rslt = bma_i2c_read_block(client,
			BMI160_USER_INTR_ENABLE_2_STEP_DETECTOR_ENABLE__REG, &v_data_u8, 1);
			*v_step_intr_u8 = BMA_GET_BITSLICE(v_data_u8,
			BMI160_USER_INTR_ENABLE_2_STEP_DETECTOR_ENABLE);
	return com_rslt;
}

static int  bmi160_set_step_detector_enable(struct i2c_client *client,u8 v_step_intr_u8)
{
	u8 com_rslt = 0;
	u8 v_data_u8 = 0;
		...
		&v_data_u8, 1);
		//if (com_rslt == SUCCESS) {
			v_data_u8 = BMA_SET_BITSLICE(v_data_u8,
			BMI160_USER_INTR_ENABLE_2_STEP_DETECTOR_ENABLE,
			v_step_intr_u8);
			com_rslt +=bma_i2c_write_block(client,
			BMI160_USER_INTR_ENABLE_2_STEP_DETECTOR_ENABLE__REG,
...
static int bmi160_get_step_counter_enable(struct i2c_client *client,u8 *v_step_counter_u8)
{
	u8 com_rslt  = 0;
	u8 v_data_u8 = 0;
			...
			BMI160_USER_STEP_CONFIG_1_STEP_COUNT_ENABLE__REG,
			&v_data_u8, 1);
			*v_step_counter_u8 = BMA_GET_BITSLICE(v_data_u8,
			BMI160_USER_STEP_CONFIG_1_STEP_COUNT_ENABLE);
		
	return com_rslt;
}

static int  bmi160_set_intr_low_g(struct i2c_client *client,u8 v_channel_u8, u8 v_intr_low_g_u8)
{
u8 com_rslt = 0;
u8 v_data_u8 = 0;
u8 v_step_cnt_stat_u8 = 0;
u8 v_step_det_stat_u8 = 0;

	com_rslt = bmi160_get_step_detector_enable(client,&v_step_det_stat_u8);

	if (v_step_det_stat_u8 != 0)
		com_rslt += bmi160_set_step_detector_enable(client,0);

	com_rslt += bmi160_get_step_counter_enable(client,&v_step_cnt_stat_u8);
	if (v_step_cnt_stat_u8 != 0)
			com_rslt += bmi160_set_step_counter_enable(client,
			0);
	switch (v_channel_u8) {
	case 0:
		com_rslt = bma_i2c_read_block(client, BMI160_USER_INTR_MAP_0_INTR1_LOW_G__REG,
		&v_data_u8, 1);
		//if (com_rslt == SUCCESS) {
			v_data_u8 = BMA_SET_BITSLICE(v_data_u8,
			BMI160_USER_INTR_MAP_0_INTR1_LOW_G, v_intr_low_g_u8);
			com_rslt += bma_i2c_write_block(client, BMI160_USER_INTR_MAP_0_INTR1_LOW_G__REG,
			&v_data_u8, 1);
		//}
		break;
	case 1:
		com_rslt =bma_i2c_read_block(client,BMI160_USER_INTR_MAP_2_INTR2_LOW_G__REG,
		&v_data_u8, 1);
		//if (com_rslt == SUCCESS) {
			v_data_u8 = BMA_SET_BITSLICE(v_data_u8,
			BMI160_USER_INTR_MAP_2_INTR2_LOW_G, v_intr_low_g_u8);
			com_rslt += bma_i2c_write_block(client, BMI160_USER_INTR_MAP_2_INTR2_LOW_G__REG,
			&v_data_u8, 1);
		//}
		break;
	default:
...
static int bmi160_set_step_config(struct i2c_client *client,u16 v_step_config_u16)
{
struct bma_i2c_data *obj = i2c_get_clientdata(client);
	u8  com_rslt = 0;
	u8 v_data1_u8r = 0;
...
mutex_lock(&obj->lock);
         ...
         bmi160_set_intr_low_g(client,0,1);
	/* write the 0 to 7 bit*/
	v_data1_u8r = (u8)(v_step_config_u16 & 0x00FF);
	com_rslt = bma_i2c_write_block(client,
			BMI160_USER_STEP_CONFIG_ZERO__REG, &v_data1_u8r, 1);
		
	/* write the 8 to 10 bit*/
	com_rslt = bma_i2c_read_block(client,
			BMI160_USER_STEP_CONFIG_ONE_CNF1__REG, &v_data2_u8r, 1);
	//if (com_rslt == SUCCESS) 
		{
		v_data3_u16 = (u16) (v_step_config_u16 & 0x0700);
		v_data1_u8r = (u8)(v_data3_u16 >> 8);
		v_data2_u8r = BMA_SET_BITSLICE(v_data2_u8r,
	...
	com_rslt = bma_i2c_read_block(client,
			BMI160_USER_STEP_CONFIG_ONE_CNF2__REG, &v_data2_u8r, 1);
	//if (com_rslt == SUCCESS) 
		{
		v_data3_u16 = (u16) (v_step_config_u16 & 0xF000);
		v_data1_u8r = (u8)(v_data3_u16 >> 12);
		v_data2_u8r = BMA_SET_BITSLICE(v_data2_u8r,
		...
		enum BMA_DATARATE_ENUM datarate)
{
	...
	if (obj->sensor_type == BMA220_TYPE) {/* BMA220 */
		...
		err += bma_i2c_write_block(client,
			BMA220_SC_FILT_CONFIG__REG, &data, 1);
		mdelay(1);
	} else if (obj->sensor_type == BMI160_TYPE) {/* BMA222 */
		if (datarate == BMA_DATARATE_10HZ)
...
static int bma_init_client(struct i2c_client *client, int reset_cali)
{
	...
	if (err < 0) {
		printk("get chip type failed, err = %d\n", err);
		return err;
	}
	//gsensor
      tmp = 0x11;
...
//gyr
	...
	//0x1d 0x07
	tmp = 0x15;//0x05;
	 err = bma_i2c_write_block(client,0x7a,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	mdelay(100);
	tmp = 0x0b;//0B
	 err = bma_i2c_write_block(client,0x7b,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	...
	//tap
	tmp = 0x07;
	 err = bma_i2c_write_block(client,0x63,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
       mdelay(30);
	tmp =0x00;
	 err = bma_i2c_write_block(client,0x64,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	...
	tmp =0x10;
	 err = bma_i2c_write_block(client,0x50,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	...
	tmp = 0x00;
	err = bma_i2c_write_block(client,0x51,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x00;
	err = bma_i2c_write_block(client,0x52,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x08;
	err = bma_i2c_write_block(client,0x53,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x00;
	err = bma_i2c_write_block(client,0x54,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x10;//0x10;
	err = bma_i2c_write_block(client,0x55,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x00;
	err = bma_i2c_write_block(client,0x56,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	tmp = 0x00;
	err = bma_i2c_write_block(client,0x57,&tmp,0x1);
	if(err < 0)
	{
		return err;
	}
	...
	//	GSE_ERR("set power mode failed, err = %d\n", err);
		//return err;
	//}
      // bmi160_set_step_config(client,0x0315);
	if (0 != reset_cali) {
...
*/
#if 0//def CUST_GYR_SENSOR
static int bma_read_gyrsensor_data(struct i2c_client *client,
		char *buf, int bufsize)
{
	...
	err = bma_read_gyrraw_data(client, databuf);
	if (err) {
...
static ssize_t show_sensordata_value(struct device_driver *ddri, char *buf)
{
	...
	//bma_read_gyrsensor_data(obj->client, strbuf, BMA_BUFSIZE);
	return snprintf(buf, PAGE_SIZE, "%s\n", strbuf);
}
...
#if 0//def CUST_GYR_SENSOR
static int gyroscope_operate(void *self, uint32_t command, void *buff_in, int size_in,
		void *buff_out, int size_out, int *actualout)
{
	int err = 0;
	int value, sample_delay;
	...
	if ((buff_out == NULL) || (size_out < sizeof(hwm_sensor_data))) {
		...
		bma_read_gyrsensor_data(priv->client, buff, BMA_BUFSIZE);
		...
		gyroscope_data->value_divide = 100;//heyong DEGREE_TO_RAD;
	}
	break;
	default:
		...
		const struct i2c_device_id *id)
{
	...
	sobj_gyr.self = obj;
	sobj_gyr.polling = 1;
	sobj_gyr.sensor_operate = gyroscope_operate;

	err = hwmsen_attach(ID_GYROSCOPE, &sobj_gyr);
#endif	
	if (err) {
