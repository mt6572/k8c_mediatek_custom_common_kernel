...
static struct i2c_board_info __initdata i2c_bma250={ I2C_BOARD_INFO("BMA250", 0x12)};
...
static int BMA250_ReadData(struct i2c_client *client, s16 data[BMA250_AXES_NUM])
{
	...
	u8 addr = 0x01;//BMA250_REG_DATAXLOW;
	...
	else
	{
	     ...
	     printk("[%08X %08X %08X] => [%5d %5d %5d] after\n", buf[BMA250_AXIS_X], buf[BMA250_AXIS_Y], buf[BMA250_AXIS_Z],
		                               buf[BMA250_AXIS_X], buf[BMA250_AXIS_Y], buf[BMA250_AXIS_Z]);
...
static int BMA250_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	databuf[0] = BMA250_REG_POWER_CTL;
      ...
      printk("bma250 set power %x,%x ",databuf[1],databuf[0]);
...
static int bma250_init_client(struct i2c_client *client, int reset_cali)
{
	...
	res = BMA250_SetDataFormat(client, 0x01);
	if(res != BMA250_SUCCESS) 
	{
		return res;
	}
...
static int BMA250_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	else
	{
		...
		printk("bma250 get data %d,%d,%d",acc[BMA250_AXIS_X], acc[BMA250_AXIS_Y], acc[BMA250_AXIS_Z]);
...
//		u8 databuf[10];    
	...
	printk(" heyong Step_init = %d\n", res);
		res = hwmsen_write_byte(client, 0x10, 0x0c);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x11, 0x8a);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x12, 0x8c);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x13, 0x20);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x14, 0x11);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x15, 0x0f);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x16, 0x08);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x32, 0x08);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x0f, 0x01);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x27, 0x32);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x28, 0x32);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
		res = hwmsen_write_byte(client, 0x29, 0x32);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
...
{ //disable step
 res = hwmsen_write_byte(client, 0x13, 0xa0);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}
...
res = hwmsen_write_byte(client, 0x13, 0x20);
	...
	mdelay(20);	
  res = hwmsen_write_byte(client, 0x16, 0x00);
		if(res != BMA250_SUCCESS) 
		{
			return res;
	...
	mdelay(20);	
   /*res = hwmsen_write_byte(client, 0x11, 0x00);
		if(res != BMA250_SUCCESS) 
		{
			return res;
		}*/
...
static int Step_ReadSensorData(struct i2c_client *client, char *buf, int bufsize)
{
	...
	if((res = hwmsen_read_block(client, 0x07, buf_step, 0x02)))
	{
		GSE_ERR("error: %d\n", res);
	}
	else
	{
	   //*data = buf_step[0] + buf_step[1] * 16 * 16;
	...
	switch (command)
	{
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				Step_ReadSensorData(priv->client, buff, BMA250_BUFSIZE);
