...
#if GTP_GESTURE_WAKEUP
static int touch_event_switch_handler(u8 key)
{
	int temp_key = 0;
	...
	switch(key)
	{
        ...
        input_report_key(tpd->dev, temp_key, 1);
        input_sync(tpd->dev);
        input_report_key(tpd->dev, temp_key, 0);
        input_sync(tpd->dev);
}
#endif
static int touch_event_handler(void *unused)
   ...
   u8  rqst_data[3] = {(u8)(GTP_REG_RQST >> 8), (u8)(GTP_REG_RQST & 0xFF), 0};
    ...
    do
    {
        ...
        if (DOZE_ENABLED == doze_status)
        {
			...
			touch_event_switch_handler(doze_buf[2]);
                    // clear 0x814B
                    doze_buf[2] = 0x00;
...
#ifdef TPD_HAVE_BUTTON //add zhumw 20140306
	   ...
	   if (key_value || pre_key)
	   {
		   ...
		   for (i = 0; i < TPD_KEY_COUNT; i++)
		   {
			   ...
			   {
				   input_x = tpd_keys_dim_local[i][0];
				   input_y = tpd_keys_dim_local[i][1];
