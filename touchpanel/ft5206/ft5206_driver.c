		...
		for(i = 0; i < point_num; i++)
		{
	       ...
	       /*get the X coordinate, 2 bytes*/
			...
			low_byte = data[3+6*i + 1];
                     #if 1
                     cinfo->x[i] =320 - (high_byte |low_byte);
                     #else
			...
			low_byte = data[3+6*i+3];
                  #if 0
                     lcd_h = high_byte |low_byte;
                     if(lcd_h>240)
                       {
			cinfo->y[i] = high_byte |low_byte;
                     cinfo->x[i] =240 - cinfo->x[i];
                       }
                     else
	...
	if((i2c_smbus_write_i2c_block_data(i2c_client, 0x88, 1, &report_rate))< 0)
	{
	    if((i2c_smbus_write_i2c_block_data(i2c_client, 0x88, 1, &report_rate))< 0)
	    {
		   TPD_DMESG("I2C read report rate error, line: %d\n", __LINE__);
		    ...
		    return -1;  //add zhumw 20131101 msg2133 add ft5206 
	}
