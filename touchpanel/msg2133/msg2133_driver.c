...
#define LCT_ADD_TP_VERSION		0	//gpg add 
#define REPORT_PACKET_LENGTH    (8)
 
#define REVERSE_Y  

#define MS_TS_MSG21XX_X_MAX   (240)
#define MS_TS_MSG21XX_Y_MAX   (240)
...
/*adair:0777为打开apk升级功能�?664为关闭apk升级功能，无需将宏__FIRMWARE_UPDATE__关闭*/
...
#define N_BYTE_PER_TIME (8)//adair:1024的约�?根据平台修改
...
static int tpd_touchinfo(TouchScreenInfo_t *touchData)
{
    ...
    if( ( Checksum == val[7] ) && ( val[0] == 0x52 ) ) //check the checksum of packet
    {
        ...
        if( ( val[1] == 0xFF ) && ( val[2] == 0xFF ) && ( val[3] == 0xFF ) && \
        	( val[4] == 0xFF ) && ( val[6] == 0xFF ) )
        {  
...
static int msg2133_enable_ps(int enable)
{
	...
	if(enable == 1)
	{
		...
		printk("msg2133 do enable: %d, current state: %d, ps_store_data[2] = 0x%x\n", enable, g_bPsSensorOpen, ps_store_data[2]);
...
static void tpd_resume( struct early_suspend *h )
{
	...
	if(g_bSuspend)         //if is tp suspend
	{
