  ...
  struct Upgrade_Info fts_updateinfo[] =
{
	...
	{0x14,"FT5336",TPD_MAX_POINTS_5,AUTO_CLB_NEED,20, 20, 0x79, 0x11, 10, 2000},
...
struct task_struct *thread_updata = NULL;
...
static  void tpd_down(int x, int y, int p) {
	...
	fts_read_reg(i2c_client, FT_REG_FW_VER, &uc_tp_fm_ver);
	printk("ft5336 fw_ver = 0x%x\n",uc_tp_fm_ver);
...
static int rockioo_touch(int x, int y)
{
  ...
  /*else if((x_0 == 0)&&(y_0 == 0)&&((x>313)||(x<7)))
 	{
	 y_0=y;
	 x_0 = x;
    	}
 else if((x_1 == 0)&&(x_0!=0)&&(ABS(x-x_0)>50)&&((x_0<7)||(x_0>313)))
 	{
 	 x_1 = x;
	 y_1 = y;
 	}*/
   else  if((x_1 == 0)&&(x_0!=0)&&(ABS(x-x_0)>100)&&(x_0<10))
 	 {
 	 x_1 = x;
	 y_1 = y;
	 }
    
//printk(" tpd xxxxxx= %d,  %d\n", x_0,x_1);	
//printk(" tpd yyyyyy= %d,  %d\n", y_0,y_1);	
   
    if(((x_1-x_0)>100)&&(x_0<10)&&(ABS(y-y_0)<30))
     	{
     	   input_set_capability(tpd->dev, EV_KEY, KEY_MENU);
	   input_report_key(tpd->dev, KEY_MENU, 1);//KEY_U, 1);
	   input_sync(tpd->dev);
	   input_report_key(tpd->dev, KEY_MENU, 0);
	   input_sync(tpd->dev);
	   rockioo_reset();
	   //printk(" tpd keycode= 3");
	   return 1;
     	  }
	return 0;
}
...
char data[128] = {0};
	...
	for(i = 0; i < point_num; i++)  
	{
		...
		cinfo->x[i] =240 - (high_byte |low_byte);
		...
		cinfo->y[i] =240 - (high_byte |low_byte);
	...
	do
	{
		...
		if (tpd_touchinfo(&cinfo, &pinfo)) 
		{
			...
			if(point_num >0) 
			{
			    ...
			    {
			        //if(rockioo_touch(cinfo.x[0], cinfo.y[0])==0)	
...
static void tpd_eint_interrupt_handler(void)
{
	printk("TPD ft5336 interrupt has been triggered\n");
	TPD_DEBUG_PRINT_INT;
	tpd_flag = 1;
...
reset_proc:   
 ...
 //   tpd_mstar_status =0 ;  // compatible mstar and ft6306 chenzhecong
	...
	thread_updata= kthread_run(touch_update_handler, 0, "tp_updata");
	if (IS_ERR(thread_updata))
	{ 
		retval = PTR_ERR(thread_updata);
		TPD_DMESG(TPD_DEVICE " failed to create kernel thread_updata: %d\n", retval);
	}
	//fts_ctpm_auto_upgrade(i2c_client);
	#endif
