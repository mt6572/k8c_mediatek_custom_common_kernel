...
static unsigned char CTPM_FW[] = {
	//#include "FT5336_Omate_Lcd0x5D_Ver0x81_20131116_app.i"
	#include "FT5336_Omate_Lcd0x5D_Ver0x87_20131127_app.i"
};
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	//     uc_tp_fm_ver < uc_host_fm_ver
	//    ) 
	if((uc_tp_fm_ver != uc_host_fm_ver)&&(uc_tp_fm_ver > 0x80))
	     {
		msleep(100);
		printk("[FTS]111 uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
				uc_tp_fm_ver, uc_host_fm_ver);
			  ...
			  u32 dw_lenth)
{
	...
	printk("ft5336 dw_lenth = %ld,packet_number = %d\n",dw_lenth,packet_number);
	if ((dw_lenth) % FTS_PACKET_LENGTH > 0) {
		temp = packet_number * FTS_PACKET_LENGTH;
		packet_buf[2] = (u8) (temp >> 8);
		packet_buf[3] = (u8) temp;
...
#else
	/*send the last six byte*/
printk("ft5336---------is_5336_new_bootloader = %d\n",is_5336_new_bootloader);
	if(is_5336_new_bootloader == BL_VERSION_LZ4 || is_5336_new_bootloader == BL_VERSION_Z7 )
	{
		for (i = 0; i<6; i++)
		{
